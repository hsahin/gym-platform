...
describe("marketing content", () => {
  it("builds a landing experience for both owners and members", () => {
    ...
    expect(content.ownerSectionTitle).toBe("Voor gymeigenaren");
    expect(content.memberSectionTitle).toBe("Voor leden");
    expect(content.primaryCta).toBe("Plan je livegang");
  ...
  it("turns an empty tenant into a launch-oriented landing instead of dead zero proof", () => {
    ...
    expect(content.ownerStage.statusLabel).toBe("Startinrichting");
    expect(content.ownerStage.primaryMetric.value).toBe("Eerste les");
    expect(content.ownerStage.secondaryMetric.value).toBe("Ledenklaar");
  ...
  it("builds owner login momentum for both setup states", () => {
    ...
    expect(setupContent.heroTitle).toBe("Van eerste vestiging tot volle lesroosters.");
    ...
    expect(liveContent.bookingPromise).toContain("sterke reserveringservaring");
  });
