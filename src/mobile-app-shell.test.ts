...
describe("native member app shell", () => {
  ...
  it("keeps the packaged mobile shell as an app-like member experience, not a single website link", () => {
    ...
    expect(source).toContain('data-open-portal="/reserve#mijn-reserveringen"');
    expect(source).toContain('data-open-portal="/reserve#contracten-betalingen"');
    expect(source).toContain('data-open-portal="/reserve#ledenservice"');
    ...
    expect(source).toContain("GymOSNativeTestHooks");
    expect(source).toContain("handleAppResume");
    expect(source).toContain("simulateNetworkForQa");
    expect(source).toContain("Betaling onderbroken of bankapp gesloten");
